...
describe("computeOrgExposureScore", () => {
  it("0 quand rien de pertinent (ni ouvert ni remédié)", () => {
    expect(
      computeOrgExposureScore({ exposedCount: 0, remediatedCount: 0 }),
    ).toBe(0);
  });

  it("0 quand tout est remédié (aucune ouverte)", () => {
    expect(
      computeOrgExposureScore({ exposedCount: 0, remediatedCount: 10 }),
    ).toBe(0);
  });

  it("100 quand beaucoup d'ouvertes et aucune remédiation (volume saturé)", () => {
    expect(
      computeOrgExposureScore({ exposedCount: 20, remediatedCount: 0 }),
    ).toBe(100);
  });
  ...
  it("50 à mi-chemin (moitié ouverte, volume mi-saturé)", () => {
    ...
    expect(
      computeOrgExposureScore({ exposedCount: 10, remediatedCount: 10 }),
    ).toBe(50);
  });

  it("décroît à mesure que la remédiation progresse (à volume égal)", () => {
    const before = computeOrgExposureScore({
      exposedCount: 10,
      remediatedCount: 0,
    });
    const after = computeOrgExposureScore({
      exposedCount: 10,
      remediatedCount: 10,
    });
  ...
  it("borne le score dans [0,100]", () => {
    const s = computeOrgExposureScore({
      exposedCount: 9999,
      remediatedCount: 0,
    });
...
describe("toCefLine", () => {
  ...
  it("produit un en-tête CEF conforme avec la sévérité du statut", () => {
    ...
    expect(
      line.startsWith("CEF:0|Humanix|Academie|1.0|EMPLOYEE_EXPOSURE|"),
    ).toBe(true);
    // NEW -> severite 6
