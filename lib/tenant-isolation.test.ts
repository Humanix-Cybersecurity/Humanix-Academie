...
vi.mock("@/lib/audit", () => ({
  ...
  AuditOutcomes: new Proxy({}, { get: (_t, prop) => String(prop) }),
}));
...
const actions = (actionsMod ?? ({} as unknown)) as NonNullable<
  typeof actionsMod
>;
...
describe.skipIf(!SUITE_AVAILABLE)(
  "Isolation tenant : actions sur Group cross-tenant",
  () => {
...
describe.skipIf(!SUITE_AVAILABLE)(
  "Isolation tenant : setUserGroups filtre les group IDs hostiles",
  () => {
...
describe.skipIf(!SUITE_AVAILABLE)(
  "Isolation tenant : protection self (anti-self-foot-shot)",
  () => {
