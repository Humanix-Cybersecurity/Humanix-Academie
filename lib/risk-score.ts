...
export type RiskFactors = {
  ...
  components: { label: string; delta: number; weight: "high" | "medium" | "low" }[];
  finalScore: number;
...
export async function computeRiskScore(userId: string): Promise<RiskFactors> {
  const user = await db.user.findUnique({
    ...
    include: {
      ...
      tenant: {
        select: {
          saisonConfigs: { where: { isMandatory: true }, select: { saisonId: true } },
        },
      },
    },
  });
  ...
  if (completionBonus > 0) {
    components.push({ label: `${completed} épisodes complétés`, delta: completionBonus, weight: "medium" });
  ...
  if (user.progress.length > 0) {
    ...
    if (avgQuizPct >= 75) {
      components.push({ label: "Score moyen quiz élevé", delta: 10, weight: "medium" });
      score += 10;
    } else if (avgQuizPct < 50) {
      components.push({ label: "Score moyen quiz faible", delta: -15, weight: "high" });
      score -= 15;
    }
  }
  ...
  if (lastActivity) {
    ...
    if (daysSince > 60) {
      components.push({ label: `Inactivité ${daysSince} jours`, delta: -25, weight: "high" });
      score -= 25;
    } else if (daysSince > 30) {
      components.push({ label: `Inactivité ${daysSince} jours`, delta: -10, weight: "medium" });
      score -= 10;
    }
  } else if (user.progress.length === 0) {
    components.push({ label: "Aucune progression", delta: -20, weight: "high" });
    score -= 20;
  }

  // 4. Modules obligatoires non complétés
  const mandatorySaisonIds = new Set(user.tenant.saisonConfigs.map((c) => c.saisonId));
  if (mandatorySaisonIds.size > 0) {
    const completedSaisonIds = new Set(user.progress.map((p) => p.saisonId));
    const missingMandatory = [...mandatorySaisonIds].filter((id) => !completedSaisonIds.has(id)).length;
  ...
  const phishingClicked = user.phishingResults.filter((r) => r.status === "CLICKED").length;
  const phishingReported = user.phishingResults.filter((r) => r.status === "REPORTED").length;
  if (phishingClicked > 0) {
    const malus = -10 * phishingClicked;
    components.push({ label: `${phishingClicked} clic(s) phishing simulé`, delta: malus, weight: "high" });
    score += malus;
  }
  if (phishingReported > 0) {
    const bonus = 5 * phishingReported;
    components.push({ label: `${phishingReported} signalement(s) phishing`, delta: bonus, weight: "medium" });
    score += bonus;
  }

  // 6. Niveau atteint : Niveau 4-5 = +5
  if (user.level >= 4) {
    components.push({ label: `Niveau ${user.level} atteint`, delta: 5, weight: "low" });
    score += 5;
  }
  ...
  else if (finalScore >= VERDICT_THRESHOLDS.a_surveiller) verdict = "a_surveiller";
...
export const RISK_VERDICT_LABEL: Record<RiskFactors["verdict"], { label: string; color: string }> = {
