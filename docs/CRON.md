...
| 12  | `/api/cron/rapport-comite-direction` | 1×/mois      | `0 7 1 * *`   | 60s    | ✅         | ⭐       | Envoie aux ADMIN de chaque tenant un **lien** vers le rapport de conformité du mois écoulé. Jamais de pièce jointe : l'accès reste authentifié et journalisé (cf. `lib/rapports/comite-direction.ts`).                                                              |
