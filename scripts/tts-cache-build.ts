...
const CONCURRENCY = Math.max(
  1,
  Math.min(20, parseInt(arg("--concurrency", "5") || "5", 10)),
);
...
function buildJobList(): Job[] {
  ...
  if (SAISON_FILTER && !listSaisons().includes(SAISON_FILTER)) {
    console.error(
      c.red(
        `Saison "${SAISON_FILTER}" introuvable. Saisons dispo : ${listSaisons().join(", ")}`,
      ),
    );
    process.exit(2);
  }
...
// Pool d'execution
// -----------------------------------------------------------------------------
async function runJobs(jobs: Job[]): Promise<{
  ok: number;
  skipped: number;
  failed: number;
  bytes: number;
  ...
  for (const j of jobs.filter((x) => x.cached)) {
    ...
    if (!manifest.segments[key] || manifest.segments[key].hash !== j.hash) {
      const stat = statSync(
        join(cacheRoot.root, j.hash.slice(0, 2), `${j.hash}.mp3`),
      );
  ...
  if (DRY_RUN) {
    ...
    for (const j of todo.slice(0, 30)) {
      console.log(
        `  ${c.dim("would gen")}  ${j.saisonSlug}/${j.episodeSlug}/${j.segment.id.padEnd(22)} voice=${j.segment.voice.padEnd(20)} (${countWords(j.segment.text)} mots)`,
      );
    }
    if (todo.length > 30)
      console.log(c.dim(`  ... et ${todo.length - 30} autres`));
    saveManifest(cacheRoot, {
      ...manifest,
  ...
  async function worker(workerId: number) {
    while (queue.length > 0) {
      ...
      try {
        ...
        console.log(
          `  ${c.green("✓")} [${pct.padStart(5)}%] ${tag.padEnd(56)} ${(r.buffer.length / 1024).toFixed(0)} Ko en ${r.latencyMs} ms ${c.dim("(w" + workerId + ")")}`,
        );
      } catch (e) {
        stats.failed++;
        processed++;
        const msg =
          e instanceof MistralTTSError
            ? `HTTP ${e.status}`
            : (e as Error).message;
...
async function main() {
  ...
  if (declaredProvider === "piper") {
    ...
    console.error(
      `   En mode Piper, le runtime synthetise lui-meme au fil des requetes.`,
    );
    console.error(
      `   Si tu veux pre-generer en Voxtral : exporte TTS_PROVIDER=voxtral + MISTRAL_API_KEY.\n`,
    );
    process.exit(1);
  }
  ...
  console.log(
    c.dim(`    model=${TTS_MODEL}  cache=${defaultCacheRoot().root}`),
  );
  ...
  console.log(
    c.dim(
      `    ${jobs.length} segments planifies sur ${new Set(jobs.map((j) => `${j.saisonSlug}/${j.episodeSlug}`)).size} episodes\n`,
    ),
  );
  ...
  console.log(c.bold(`\n📊 Resume :`));
  console.log(
    `   ${c.green(`${stats.ok} OK`)}  ${c.yellow(`${stats.skipped} skipped`)}  ${stats.failed > 0 ? c.red(`${stats.failed} failed`) : c.dim("0 failed")}`,
  );
  if (stats.bytes > 0) {
    console.log(
      `   ${(stats.bytes / 1024 / 1024).toFixed(1)} Mo audio genere, ${(stats.ms / 1000).toFixed(0)} sec total temps API`,
    );
    if (stats.ok > 0) {
      console.log(
        `   moyenne : ${(stats.ms / stats.ok).toFixed(0)} ms / segment`,
      );
    }
  }
  if (stats.failed > 0) {
