...
export default function ApiKeysManager({ keys }: { keys: Key[] }) {
  ...
  const [newlyCreated, setNewlyCreated] = useState<{ plain: string; name: string } | null>(null);
  ...
  const onCreate = async (formData: FormData) => {
    startTransition(async () => {
      try {
        ...
        setNewlyCreated(res);
        const form = document.querySelector("#new-key-form") as HTMLFormElement | null;
        form?.reset();
      } catch {
  ...
  const onRevoke = (id: string) => {
    if (!confirm("Révoquer cette clé ? Les intégrations qui l'utilisent vont cesser de fonctionner.")) return;
    startTransition(async () => {
      try {
    ...
    <>
      {newlyCreated && (
        <div className="card mb-6 bg-success/10 border-success">
          <h3 className="font-bold text-success mb-2">🎉 Clé "{newlyCreated.name}" créée</h3>
          <p className="text-sm text-gray-700 mb-3">
            <strong>Copie-la maintenant</strong> — elle ne sera plus jamais affichée en clair.
          </p>
          ...
          <button onClick={() => setNewlyCreated(null)} className="text-xs text-gray-500 mt-3 hover:text-warn">
      ...
      <div className="card mb-6">
        <h3 className="font-bold text-primary-500 mb-4">Créer une nouvelle clé</h3>
        <form id="new-key-form" action={onCreate} className="grid sm:grid-cols-3 gap-3">
          <input
            name="name"
            type="text"
            required
          ...
          />
          <select name="expiresInDays" className="rounded-xl border-2 border-gray-200 p-3 text-sm bg-white">
            <option value="0">Sans expiration</option>
            <option value="30">Expire dans 30 jours</option>
            <option value="90" selected>Expire dans 90 jours</option>
        ...
        <div className="card">
          <table className="w-full text-sm">
            <thead className="text-left text-gray-500 border-b">
              <tr>
                <th scope="col" className="pb-3">Nom</th>
                <th scope="col" className="pb-3">Préfixe</th>
                <th scope="col" className="pb-3">Statut</th>
                <th scope="col" className="pb-3">Dernière utilisation</th>
                <th scope="col" className="pb-3">Expire</th>
                ...
                <tr key={k.id} className="border-b last:border-0">
                  ...
                  <td className="py-3">
                    {k.isActive ? (
                      ...
                      <span className="text-xs bg-gray-200 text-gray-600 px-2 py-1 rounded-full">RÉVOQUÉE</span>
                    )}
                  </td>
                  ...
                  <td className="py-3 text-xs text-gray-500">
                    {k.expiresAt ? new Date(k.expiresAt).toLocaleDateString("fr-FR") : "Sans"}
                  </td>
