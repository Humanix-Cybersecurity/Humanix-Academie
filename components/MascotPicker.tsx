...
export default function MascotPicker({
  ...
  const onChoose = (id: string, locked: boolean) => {
    if (locked) {
      setFeedback({ type: "err", msg: "Cette mascotte se débloque à un niveau supérieur." });
      return;
    }
    setFeedback(null);
    setPickedId(id);
  ...
  const onSave = () => {
    ...
    startTransition(async () => {
      try {
        ...
        setFeedback({ type: "ok", msg: "✓ Ta mascotte a changé. Bienvenue à elle !" });
  ...
  return (
    <div className="grid lg:grid-cols-[1fr_2fr] gap-6">
      ...
      <div className="card sticky top-20 self-start">
        <p className="text-xs uppercase tracking-wide text-gray-500 font-bold mb-3">Aperçu</p>
        ...
        <p className="text-center font-bold text-primary-500 text-lg">{picked.name}</p>
        <p className="text-center text-sm text-gray-600 italic mb-4">{picked.tagline}</p>
        <button
          onClick={onSave}
          disabled={pending || pickedId === currentMascotId}
          className="btn-primary w-full"
        >
          {pickedId === currentMascotId ? "Mascotte actuelle" : pending ? "Sauvegarde…" : "✓ Adopter cette mascotte"}
        </button>
        {feedback && (
          <p className={`text-sm text-center mt-3 font-medium ${feedback.type === "ok" ? "text-success" : "text-warn"}`}>
            {feedback.msg}
          </p>
        )}
      </div>
          ...
          {allSpecies.map((m) => {
            ...
            return (
              ...
              >
                ...
                <p className="text-sm font-bold text-primary-500 truncate">{m.name}</p>
                {locked && (
                  <p className="text-[10px] text-gray-500 mt-1">Niv. {m.unlockLevel} requis</p>
                )}
                {!locked && m.unlockLevel > 0 && (
                  <p className="text-[10px] text-gray-500 mt-1">Débloqué Niv. {m.unlockLevel}</p>
                )}
              </button>
            );
          })}
