...
type SaisonRow = {
  name: string; emoji: string; completed: number; total: number; pct: number;
};

type TeamRow = {
  name: string; service: string; episodesDone: number; totalEpisodes: number;
  xp: number; lastActivity: string | null;
};
...
const LEVEL_META: Record<Level, {
  ...
  excellent: { label: "Excellent",    text: "text-emerald-700 dark:text-emerald-300", bg: "bg-emerald-50 dark:bg-emerald-900/20", bar: "bg-emerald-500", hex: "#10b981" },
  ok:        { label: "Correct",      text: "text-teal-700 dark:text-teal-300",       bg: "bg-teal-50 dark:bg-teal-900/20",       bar: "bg-teal-500",    hex: "#14b8a6" },
  warning:   { label: "À surveiller", text: "text-amber-700 dark:text-amber-300",     bg: "bg-amber-50 dark:bg-amber-900/20",     bar: "bg-amber-500",   hex: "#f59e0b" },
  danger:    { label: "Critique",     text: "text-rose-700 dark:text-rose-300",       bg: "bg-rose-50 dark:bg-rose-900/20",       bar: "bg-rose-500",    hex: "#f43f5e" },
};
...
export default function AdminDashboard({
  stats, saisonsBreakdown, teamProgress, weeklyActivity,
}: Props) {
  ...
  return (
    <div className="space-y-6 min-w-0">
      ...
      {recommendations.length > 0 && <UrgentActions actions={recommendations} />}
      ...
      <details className="group rounded-xl border border-gray-200 dark:border-slate-800 bg-white dark:bg-slate-900/50 px-4 py-3 text-sm">
        <summary className="cursor-pointer font-medium text-gray-600 dark:text-gray-400 list-none flex items-center gap-2 hover:text-primary-500 dark:hover:text-accent-300">
          <span className="text-gray-400 group-open:rotate-90 transition-transform" aria-hidden="true">▸</span>
        ...
        <div className="mt-3 pt-3 border-t border-gray-100 dark:border-slate-800 space-y-2 text-gray-600 dark:text-gray-400 leading-relaxed">
          <p><strong className="text-gray-900 dark:text-gray-200">Cyberscore</strong> : composite activation (40&nbsp;%) + complétion (60&nbsp;%). Indicateur de pilotage rapide.</p>
          <p><strong className="text-gray-900 dark:text-gray-200">Maîtrise cyber</strong> : moyenne du risk score individuel (0–100). Vrai indicateur de maturité humaine.</p>
          <p><strong className="text-gray-900 dark:text-gray-200">Activation</strong> : % de collaborateurs ayant lancé au moins un module. Cible ≥&nbsp;80&nbsp;%.</p>
          <p><strong className="text-gray-900 dark:text-gray-200">XP / épisode</strong> : indicateur de gamification. Peut dépasser 100. Pas un score de maîtrise.</p>
        </div>
      </details>
    </div>
  );
...
function CyberscoreHero({
  breakdown, totalSeats, activatedSeats,
}: {
        ...
        <div className="flex items-center gap-5 flex-1 min-w-0">
          <div className="relative w-24 h-24 sm:w-28 sm:h-28 shrink-0">
            <ResponsiveContainer width="100%" height="100%">
              <RadialBarChart innerRadius="78%" outerRadius="100%"
                data={[{ name: "Score", value: breakdown.score, fill: meta.hex }]}
                startAngle={90} endAngle={-270}>
                <RadialBar background={{ fill: "rgba(0,0,0,0.06)" }} dataKey="value" cornerRadius={20} />
              </RadialBarChart>
            </ResponsiveContainer>
            <div className="absolute inset-0 flex flex-col items-center justify-center leading-none">
              <span className="text-3xl sm:text-4xl font-extrabold tabular-nums text-gray-900 dark:text-gray-100">{breakdown.score}</span>
              <span className="text-[10px] uppercase tracking-widest text-gray-400 mt-0.5">/ 100</span>
            </div>
          </div>
          ...
          <div className="min-w-0">
            ...
            <p className="text-sm text-gray-600 dark:text-gray-400 mt-1.5">
              Maîtrise équipe&nbsp;: <strong className="text-gray-900 dark:text-gray-200 tabular-nums">{breakdown.components.mastery.score * 2}/100</strong>
              <span className="mx-2 text-gray-300 dark:text-slate-600" aria-hidden="true">·</span>
              Activation&nbsp;: <strong className="text-gray-900 dark:text-gray-200 tabular-nums">{activatedSeats}/{totalSeats}</strong>
            </p>
          </div>
        </div>
...
function ScoreBreakdownDetail({ breakdown }: { breakdown: CyberscoreBreakdown }) {
  return (
    <details className="group mt-5 pt-4 border-t border-gray-100 dark:border-slate-800">
      <summary className="cursor-pointer flex items-center gap-2 text-xs font-semibold text-gray-600 dark:text-gray-400 hover:text-primary-500 dark:hover:text-accent-300 list-none">
        ...
        <span className="ml-auto text-gray-400 italic">Note de sévérité&nbsp;: élevée</span>
      </summary>

      <div className="mt-4 space-y-4 text-sm">
        <p className="text-xs text-gray-500 dark:text-gray-400 leading-relaxed italic">
          En cyber, mieux vaut un score sévère qu'un score complaisant. Cette note
          intègre 3 composantes pondérées + des pénalités explicites pour refléter
          le risque réel — pas le confort de lecture.
        </p>
            ...
            <ul className="space-y-1.5 list-none">
              {breakdown.penalties.map((p, i) => (
                ...
                >
                  ...
                  <div className="min-w-0">
                    <p className="font-semibold text-rose-700 dark:text-rose-300">{p.label}</p>
                    <p className="text-rose-700/80 dark:text-rose-300/80 mt-0.5 leading-snug">{p.reason}</p>
                  </div>
                </li>
              ))}
            </ul>
        ...
        <div className="pt-3 border-t border-gray-100 dark:border-slate-800 text-xs text-gray-500 dark:text-gray-400">
          <p className="leading-relaxed">
            Score brut&nbsp;: <strong className="text-gray-700 dark:text-gray-300 tabular-nums">{breakdown.raw}/100</strong>
            <span className="mx-1.5 text-gray-300 dark:text-slate-600" aria-hidden="true">→</span>
            courbe concave (ø1.25)
            {breakdown.penalties.length > 0 && (
              <>
                <span className="mx-1.5 text-gray-300 dark:text-slate-600" aria-hidden="true">→</span>
                <span className="text-rose-600 dark:text-rose-400">−{breakdown.penalties.reduce((s, p) => s + p.points, 0)}&nbsp;pénalités</span>
              </>
            )}
            <span className="mx-1.5 text-gray-300 dark:text-slate-600" aria-hidden="true">=</span>
            <strong className="text-gray-900 dark:text-gray-100 tabular-nums">{breakdown.score}/100</strong>
          </p>
          <p className="mt-2 text-[11px]">
            Seuils&nbsp;:
            <span className="ml-1.5 text-emerald-600 dark:text-emerald-400">85+ Excellent</span> ·
            <span className="ml-1.5 text-teal-600 dark:text-teal-400">70+ Bon</span> ·
            <span className="ml-1.5 text-gray-600 dark:text-gray-400">55+ Correct</span> ·
            <span className="ml-1.5 text-amber-600 dark:text-amber-400">40+ À surveiller</span> ·
            <span className="ml-1.5 text-rose-600 dark:text-rose-400">&lt;40 Critique</span>
          </p>
        </div>
      </div>
    </details>
...
function ScoreComponent({
  score, max, label, explanation,
}: {
  score: number; max: number; label: string; explanation: string;
}) {
  ...
  return (
    <div className="flex items-center gap-3">
      <div className="flex-1 min-w-0">
        ...
        <p className="text-[11px] text-gray-500 dark:text-gray-400 mt-1 leading-snug">{explanation}</p>
      </div>
    </div>
  );
}
...
function buildRecommendations(stats: Stats, saisons: SaisonRow[], team: TeamRow[]): Action[] {
  ...
  if (stats.totalSeats > 0 && stats.masteryAverage < 60) {
    actions.push({
      ...
      description: "Sous le seuil de sécurité. Un challenge collectif aide à booster.",
      cta: { label: "Lancer un challenge", href: "/admin/challenge" },
    });
  }

  const worst = saisons.filter((s) => s.total > 0).sort((a, b) => a.pct - b.pct)[0];
  ...
  const dormants = team.filter((t) => t.lastActivity === null && t.episodesDone === 0).length;
  if (dormants >= 3) {
    actions.push({
      level: "info",
      icon: "📨",
...
const ACTION_STYLES: Record<Action["level"], { border: string; iconBg: string; titleText: string; btn: string }> = {
  danger:  { border: "border-rose-200 dark:border-rose-900/50",   iconBg: "bg-rose-50 dark:bg-rose-900/20",   titleText: "text-rose-700 dark:text-rose-300",   btn: "bg-rose-600 hover:bg-rose-700 text-white" },
  warning: { border: "border-amber-200 dark:border-amber-900/50", iconBg: "bg-amber-50 dark:bg-amber-900/20", titleText: "text-amber-700 dark:text-amber-300", btn: "bg-amber-500 hover:bg-amber-600 text-white" },
  info:    { border: "border-teal-200 dark:border-teal-900/50",   iconBg: "bg-teal-50 dark:bg-teal-900/20",   titleText: "text-teal-700 dark:text-teal-300",   btn: "bg-teal-600 hover:bg-teal-700 text-white" },
};
          ...
          return (
            ...
            >
              <div className="flex items-start gap-3 min-w-0">
                <span className={`shrink-0 w-9 h-9 rounded-lg ${s.iconBg} flex items-center justify-center text-lg`} aria-hidden="true">
                  {a.icon}
                </span>
                <div className="min-w-0">
                  <p className={`font-bold text-sm ${s.titleText} truncate`}>{a.title}</p>
                  <p className="text-xs text-gray-600 dark:text-gray-400 mt-0.5 leading-snug">{a.description}</p>
                </div>
              </div>
              <a
                href={a.cta.href}
...
function KpiStrip({ stats, weeklyActivity }: { stats: Stats; weeklyActivity: WeeklyPoint[] }) {
  const completionRate = stats.totalEpisodes * stats.totalSeats === 0
    ? 0
    : Math.round((stats.completedEpisodes / (stats.totalEpisodes * stats.totalSeats)) * 100);

  return (
    <section aria-label="Indicateurs clés" className="grid grid-cols-2 lg:grid-cols-4 gap-3">
      <KpiCard label="Sièges actifs" value={stats.totalSeats} delta={`${stats.activatedSeats} ont commencé`} progress={stats.activationRate} level={levelFromScore(stats.activationRate)} sparkline={weeklyActivity} />
      <KpiCard label="Maîtrise cyber" value={`${stats.masteryAverage}/100`} delta="Risk score moyen" progress={stats.masteryAverage} level={levelFromScore(stats.masteryAverage)} highlight />
      <KpiCard label="Modules complétés" value={stats.completedEpisodes} delta={`sur ${stats.totalEpisodes * stats.totalSeats} possibles`} progress={completionRate} level={levelFromScore(completionRate)} />
      <KpiCard label="Engagement XP" value={stats.averageXpPerEpisode} delta="moyenne / épisode" progress={Math.min(100, stats.averageXpPerEpisode / 1.5)} level={stats.averageXpPerEpisode >= 80 ? "excellent" : "ok"} />
    </section>
  );
}

function KpiCard({
  label, value, delta, progress, level, highlight, sparkline,
}: {
  label: string; value: string | number; delta: string;
  progress: number; level: Level; highlight?: boolean;
  ...
  return (
    ...
    >
      <p className="text-[10px] uppercase tracking-widest text-gray-500 dark:text-gray-400 font-bold flex items-center gap-1.5">
        {label}
        {highlight && <span className="text-accent-500" aria-hidden="true">★</span>}
      </p>
          ...
          <div className="w-16 h-8 opacity-80">
            <ResponsiveContainer width="100%" height="100%">
              <BarChart data={sparkline}>
                <Bar dataKey="completions" fill={meta.hex} radius={[2, 2, 0, 0]} />
              </BarChart>
            </ResponsiveContainer>
          </div>
        )}
      ...
      <p className="text-[11px] text-gray-500 dark:text-gray-400 mt-1 truncate">{delta}</p>
...
function ActivityChart({ data, className = "" }: { data: WeeklyPoint[]; className?: string }) {
  ...
  const bestDay = data.reduce((best, p) => p.completions > (best?.completions ?? -1) ? p : best, data[0]);
  ...
  return (
    ...
    <article className={`rounded-xl border border-gray-200 dark:border-slate-800 bg-white dark:bg-slate-900 p-4 min-w-0 ${className}`}>
      <header className="flex items-baseline justify-between mb-2 gap-2">
        <h3 className="font-bold text-gray-900 dark:text-gray-100 text-sm">Activité de la semaine</h3>
        <p className="text-xs text-gray-500 dark:text-gray-400">
          <span className="font-bold text-gray-900 dark:text-gray-100 tabular-nums">{total}</span> complétions
        </p>
      </header>
      ...
      <div className="h-[140px]">
        <ResponsiveContainer width="100%" height="100%">
          <BarChart data={data} barSize={20} margin={{ top: 4, right: 4, left: -20, bottom: 0 }}>
            <XAxis dataKey="day" tickLine={false} axisLine={false} fontSize={10} stroke="#9ca3af" />
            <YAxis tickLine={false} axisLine={false} fontSize={10} stroke="#9ca3af" allowDecimals={false} width={28} />
            <Tooltip
              cursor={{ fill: "rgba(0,163,161,0.06)" }}
              contentStyle={{ borderRadius: 8, border: "1px solid #e5e7eb", fontSize: 11 }}
              formatter={(v: number) => [`${v} épisode${v > 1 ? "s" : ""}`, "Complétions"]}
            />
            <Bar dataKey="completions" radius={[5, 5, 0, 0]}>
              {data.map((_, i) => (
                <Cell key={i} fill={i === data.length - 1 ? "#0B3D91" : "#00A3A1"} />
              ))}
            </Bar>
          </BarChart>
        </ResponsiveContainer>
      ...
      <footer className="mt-2 pt-2 border-t border-gray-100 dark:border-slate-800 flex items-center justify-between gap-2 text-[11px]">
        <span className="text-gray-500 dark:text-gray-400">
          <span className="font-bold text-gray-900 dark:text-gray-100 tabular-nums">{avgPerDay}</span>{" "}moy./jour
        </span>
        {bestDay && bestDay.completions > 0 && (
          <span className="text-gray-500 dark:text-gray-400">
            Top&nbsp;:&nbsp;
            <span className="font-bold text-gray-900 dark:text-gray-100 capitalize">{bestDay.day}</span>{" "}
            <span className="font-bold text-accent-500 tabular-nums">({bestDay.completions})</span>
          </span>
        )}
      </footer>
    </article>
...
function SaisonsCoverage({ data, className = "" }: { data: SaisonRow[]; className?: string }) {
  return (
    // h-full pour matcher la hauteur de l'ActivityChart en grid (les 2 cards
    // sont sur la même row).
    <article className={`h-full rounded-xl border border-gray-200 dark:border-slate-800 bg-white dark:bg-slate-900 p-4 sm:p-5 min-w-0 ${className}`}>
      <header className="flex items-baseline justify-between mb-4 gap-2">
        <h3 className="font-bold text-gray-900 dark:text-gray-100">Couverture par saison</h3>
        <p className="text-xs text-gray-500">{data.length} saison{data.length > 1 ? "s" : ""}</p>
      </header>
      {data.length === 0 ? (
        <p className="text-sm text-gray-400 italic text-center py-8">Aucune saison publiée.</p>
      ) : (
              ...
              <li key={s.name}>
                <div className="flex justify-between text-sm mb-1 gap-2">
                  ...
                  <span className="text-xs text-gray-500 shrink-0 tabular-nums">
                    {s.completed}/{s.total} · <span className={`font-bold ${meta.text}`}>{s.pct}%</span>
                  </span>
                </div>
                <div className="w-full h-1.5 bg-gray-100 dark:bg-slate-800 rounded-full overflow-hidden">
                  <div className={`h-full rounded-full transition-all ${meta.bar}`} style={{ width: `${Math.min(100, s.pct)}%` }} />
                </div>
              </li>
            );
          })}
...
function TopPerformers({ team, className = "" }: { team: TeamRow[]; className?: string }) {
  return (
    ...
    <article className={`rounded-xl border border-gray-200 dark:border-slate-800 bg-white dark:bg-slate-900 p-4 min-w-0 ${className}`}>
      <header className="flex items-baseline justify-between mb-3 gap-2">
        <h3 className="font-bold text-gray-900 dark:text-gray-100 text-sm">Top 3 collaborateurs</h3>
        <p className="text-[10px] text-gray-400 italic uppercase tracking-wide">Visible dirigeant</p>
      </header>
      {team.length === 0 ? (
        <p className="text-sm text-gray-400 italic text-center py-4">Aucun collaborateur actif.</p>
      ) : (
        <ol className="space-y-1.5 list-none">
          {team.slice(0, 3).map((u, i) => (
            <li key={u.name + i} className="flex items-center gap-2.5 px-1 py-1 rounded-md hover:bg-gray-50 dark:hover:bg-slate-800/40 transition">
              <span
              ...
              <div className="flex-1 min-w-0">
                <p className="font-semibold text-sm truncate text-gray-900 dark:text-gray-100 leading-tight">{u.name}</p>
                <p className="text-[11px] text-gray-500 truncate">{u.service}</p>
              </div>
              <div className="text-right shrink-0">
                <p className="font-extrabold text-accent-500 tabular-nums text-sm">{u.xp}</p>
                <p className="text-[9px] text-gray-400 uppercase tracking-wide leading-none">XP</p>
              </div>
            </li>
          ))}
        </ol>
...
function EngagementMini({
  averageXpPerEpisode, totalXP, className = "",
}: {
  averageXpPerEpisode: number; totalXP: number; className?: string;
}) {
  return (
    // Compact (stack vertical à droite, plus de h-full).
    <article className={`rounded-xl border border-amber-200 dark:border-amber-900/50 bg-gradient-to-br from-amber-50 to-orange-50 dark:from-amber-900/15 dark:to-orange-900/15 p-4 min-w-0 ${className}`}>
      <div className="flex items-center justify-between gap-3 mb-2">
        <div className="flex items-center gap-2.5 min-w-0">
          <span className="shrink-0 w-8 h-8 rounded-md bg-amber-100 dark:bg-amber-900/30 flex items-center justify-center text-base" aria-hidden="true">🎮</span>
          <div className="min-w-0">
      ...
      <div className="pt-2 border-t border-amber-200/60 dark:border-amber-800/50 flex items-center justify-between text-[11px]">
        <span className="text-amber-800/80 dark:text-amber-200/80">Total cumulé</span>
...
function TeamTable({ rows }: { rows: TeamRow[] }) {
  ...
  const filtered = useMemo(() => {
    ...
    let res = rows.filter((r) => {
      ...
      if (q && !r.name.toLowerCase().includes(q) && !r.service.toLowerCase().includes(q)) return false;
      return true;
    });
    res = [...res].sort((a, b) => {
      let cmp = 0;
      if (sortKey === "name") cmp = a.name.localeCompare(b.name, "fr");
      else if (sortKey === "service") cmp = a.service.localeCompare(b.service, "fr");
  ...
  return (
    <section aria-label="Suivi équipe complet" className="rounded-xl border border-gray-200 dark:border-slate-800 bg-white dark:bg-slate-900 p-4 sm:p-5 min-w-0">
      <header className="mb-4 space-y-3 sm:space-y-0 sm:flex sm:items-center sm:justify-between sm:gap-4">
        <div className="min-w-0">
          <h3 className="font-bold text-gray-900 dark:text-gray-100">Suivi équipe</h3>
          <p className="text-xs text-gray-500 mt-0.5">
            <span className="tabular-nums">{filtered.length}</span> / <span className="tabular-nums">{rows.length}</span> collaborateur{rows.length > 1 ? "s" : ""}
          </p>
        </div>
      ...
      <table className="w-full text-sm table-auto">
        <thead className="text-left text-gray-500 dark:text-gray-400 border-b border-gray-200 dark:border-slate-800">
          <tr>
            <Th label="Collaborateur" k="name" sortKey={sortKey} sortAsc={sortAsc} onSort={toggleSort} />
            <Th label="Service" k="service" sortKey={sortKey} sortAsc={sortAsc} onSort={toggleSort} hideUnder="md" />
            <Th label="Progression" k="progress" sortKey={sortKey} sortAsc={sortAsc} onSort={toggleSort} />
            <Th label="XP" k="xp" sortKey={sortKey} sortAsc={sortAsc} onSort={toggleSort} numeric />
            <Th label="Dernière activité" k="lastActivity" sortKey={sortKey} sortAsc={sortAsc} onSort={toggleSort} hideUnder="xl" />
          </tr>
        </thead>
        <tbody>
          {filtered.length === 0 && (
          ...
          {filtered.map((u, i) => {
            const pct = u.totalEpisodes === 0 ? 0 : Math.round((u.episodesDone / u.totalEpisodes) * 100);
            const lvl = levelFromScore(pct);
            return (
              <tr key={u.name + i} className="border-b border-gray-100 dark:border-slate-800/60 last:border-0 hover:bg-gray-50/60 dark:hover:bg-slate-800/30 transition align-middle">
                <td className="py-3 pr-3 min-w-0">
                  ...
                  <div className="md:hidden mt-0.5 text-xs text-gray-500 dark:text-gray-400 truncate">
                    {u.service}
                    <span className="mx-1.5 text-gray-300 dark:text-slate-600" aria-hidden="true">·</span>
                    <span className="xl:hidden">{u.lastActivity ?? <span className="italic">Pas connecté</span>}</span>
                  </div>
                  <div className="hidden md:block xl:hidden mt-0.5 text-[11px] text-gray-400 truncate">
                    {u.lastActivity ?? <span className="italic">Pas connecté</span>}
                  </div>
                </td>
                ...
                <td className="py-3 pr-3 whitespace-nowrap">
                  <div className="flex items-center gap-2">
                    <div className="w-16 sm:w-20 lg:w-28 h-1.5 bg-gray-100 dark:bg-slate-800 rounded-full overflow-hidden shrink-0">
                      <div className={`h-full ${LEVEL_META[lvl].bar} transition-all`} style={{ width: `${pct}%` }} />
                    </div>
                ...
                <td className="hidden xl:table-cell py-3 pl-3 text-gray-500 dark:text-gray-400 text-xs whitespace-nowrap">
                  {u.lastActivity ?? <span className="italic text-gray-400">Pas encore</span>}
                </td>
              </tr>
            );
          })}
...
function Th({ label, k, sortKey, sortAsc, onSort, numeric, hideUnder }: {
  ...
  const hideClass =
    hideUnder === "md" ? "hidden md:table-cell" :
    hideUnder === "lg" ? "hidden lg:table-cell" :
    hideUnder === "xl" ? "hidden xl:table-cell" : "";
  return (
    <th
      scope="col"
