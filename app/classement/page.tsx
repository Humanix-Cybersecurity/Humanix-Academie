...
import { getActiveChallenge, getChallengeRanking, getChallengeIndividualRanking } from "@/lib/challenge";
...
export default async function ClassementPage() {
  ...
  if (!active) {
    return (
      <div className="max-w-4xl mx-auto px-4 py-16 text-center animate-fadeIn">
        ...
        <h1 className="text-3xl font-extrabold text-primary-500 mb-2">Aucun challenge en cours</h1>
        <p className="text-gray-600 mb-6">
          Reviens quand ton dirigeant aura lancé un Cyber-Challenge !
        </p>
        <Link href="/apprendre" className="btn-primary">Retour à mes saisons</Link>
      </div>
    );
  }
  ...
  const daysLeft = Math.max(0, Math.ceil((active.endDate.getTime() - Date.now()) / (24 * 3600 * 1000)));
  ...
  return (
    <div className="max-w-5xl mx-auto px-4 py-8 animate-fadeIn">
      <div className="card mb-8 bg-gradient-to-br from-amber-100 via-orange-100 to-red-100 border-2 border-amber-400">
        <div className="flex items-start justify-between gap-4">
          <div>
            ...
            <h1 className="text-3xl sm:text-4xl font-extrabold text-primary-500">{active.title}</h1>
            {active.description && <p className="text-gray-700 mt-2">{active.description}</p>}
          </div>
          <div className="text-right">
            <p className="text-5xl font-extrabold text-orange-600">{daysLeft}</p>
            <p className="text-xs uppercase text-gray-600">jour{daysLeft > 1 ? "s" : ""} restant{daysLeft > 1 ? "s" : ""}</p>
          </div>
        </div>
      </div>
      ...
      <div className="grid lg:grid-cols-2 gap-8">
        <div>
          <h2 className="text-xl font-bold text-primary-500 mb-4">🏛️ Classement par service</h2>
          {byService.length === 0 ? (
        ...
        <div>
          <h2 className="text-xl font-bold text-primary-500 mb-4">⚡ Top 10 individuel</h2>
          {top10.length === 0 ? (
            <div className="card text-center py-8 text-gray-400 italic">Personne au tableau pour l'instant.</div>
          ) : (
            <div className="card">
              <ol className="space-y-2">
                {top10.map((u, i) => (
                  <li key={u.userId} className="flex items-center gap-3 py-2 border-b last:border-0">
                    <span className="text-xl w-8 text-center">
                      {i === 0 ? "🥇" : i === 1 ? "🥈" : i === 2 ? "🥉" : `${i + 1}`}
                    </span>
                    <div className="flex-1">
                      <p className="font-medium text-sm">{u.name}</p>
                      <p className="text-xs text-gray-500">{u.service ?? "Sans service"}</p>
                    </div>
