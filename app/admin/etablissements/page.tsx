    ...
    <div className="max-w-6xl mx-auto px-4 py-10">
      ...
      <section className="card mb-8">
        ...
        <p className="text-sm text-gray-500 mb-4">
          Indicateurs agrégés sur {stats.byEstablishment.length} établissements.
        </p>
        <div className="grid grid-cols-2 sm:grid-cols-4 gap-3">
          <KpiSmall label="Sièges actifs" value={stats.totalSeats.toString()} />
          <KpiSmall
            label="Activation"
            value={stats.totalSeats === 0 ? "0%" : `${Math.round((stats.activatedSeats / stats.totalSeats) * 100)}%`}
          />
          <KpiSmall label="Modules complétés" value={stats.completedEpisodes.toString()} />
          <KpiSmall label="Maîtrise moyenne" value={`${stats.averageMastery}/100`} />
        </div>
      </section>
          ...
          <table className="w-full border-collapse min-w-[700px]">
            <thead>
              <tr className="text-left text-xs uppercase tracking-wide text-gray-600 dark:text-gray-300 border-b border-gray-200 dark:border-slate-700">
                ...
                <th scope="col" className="p-3 text-right">Sièges</th>
                <th scope="col" className="p-3 text-right">Maîtrise</th>
                <th scope="col" className="p-3 text-right">Modules</th>
            ...
            <tbody>
              {stats.byEstablishment.map((e) => {
                ...
                return (
                  <tr key={e.tenantId} className="border-b border-gray-100 dark:border-slate-800">
                    <td className="p-3">
                    ...
                    <td className="p-3">
                      {!isRoot && (
                        <form action={deleteEstablishmentAction} className="inline">
      ...
      {canAddChildren ? (
        <section className="card">
          ...
          <p className="text-sm text-gray-500 mb-4">
            Le nouvel établissement hérite de votre offre actuelle. Vous pourrez
            ensuite y inviter ses collaborateurs.
          </p>
          <form action={createEstablishmentAction} className="grid sm:grid-cols-2 gap-4">
            <div>
              <label
                htmlFor="etab-name"
                className="block text-xs font-bold uppercase text-gray-500 mb-1"
              >
                Nom de l'établissement <span className="text-red-500" aria-hidden="true">*</span>
              </label>
              <input
                id="etab-name"
                name="name"
            ...
            <div>
              ...
              <select id="etab-type" name="establishmentType" className="input w-full">
        ...
        <section className="card bg-gray-50 dark:bg-slate-800">
          <p className="text-sm text-gray-700 dark:text-gray-300">
            <strong>Cette vue est rattachée à un établissement enfant.</strong>{" "}
