      ...
      <div className="space-y-6 min-w-0">
        ...
        <article className="rounded-xl border border-amber-200 dark:border-amber-900/50 bg-amber-50/60 dark:bg-amber-900/15 p-4">
          ...
          <p className="text-xs text-amber-800/80 dark:text-amber-200/80 mt-2 leading-relaxed">
            Les simulations doivent être <strong>annoncées préalablement</strong> aux salariés (charte, CSE).
                  ...
                  >
                    <header className="flex items-start justify-between gap-4 mb-3 flex-wrap">
                      <div className="flex items-center gap-2.5 min-w-0">
                        <span className="text-2xl shrink-0" aria-hidden="true">{tpl?.emoji ?? "🎣"}</span>
                        <div className="min-w-0">
                          <div className="flex items-center gap-2 flex-wrap">
                            <h4 className="font-bold text-gray-900 dark:text-gray-100 truncate">{c.title}</h4>
                            {c.isActive ? (
                              <StatusBadge level="success" pill>En cours</StatusBadge>
                            ) : (
                              <StatusBadge level="neutral" pill>Terminée</StatusBadge>
                            )}
                          </div>
                    ...
                    <div className="grid grid-cols-3 gap-2">
                      <ResultStat label="Cliqué"  value={clicked}  total={sent} variant="danger" />
                      <ResultStat label="Signalé" value={reported} total={sent} variant="success" />
                      <ResultStat label="Ignoré"  value={ignored}  total={sent} variant="neutral" />
                    </div>
                  </article>
                );
              })}
...
function CrossSellCard({
  badge, title, description, cta, href, icon,
}: {
  badge: string; title: string; description: string;
  cta: string; href: string; icon: string;
}) {
  return (
    <Link
      href={href}
    ...
    >
      <div className="flex items-start gap-3 min-w-0">
        <span className="shrink-0 w-10 h-10 rounded-lg bg-white/70 dark:bg-slate-900/50 flex items-center justify-center text-xl" aria-hidden="true">
          {icon}
        </span>
        <div className="min-w-0">
...
function ResultStat({
  ...
  const styles =
    variant === "danger" ? "bg-rose-50 dark:bg-rose-900/15 text-rose-700 dark:text-rose-300" :
    variant === "success" ? "bg-emerald-50 dark:bg-emerald-900/15 text-emerald-700 dark:text-emerald-300" :
  ...
  return (
    <div className={`rounded-lg p-3 ${styles}`}>
      ...
      <p className="text-[10px] uppercase tracking-widest font-bold mt-0.5">{label}</p>
    </div>
  );
}
