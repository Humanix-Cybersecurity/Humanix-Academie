  ...
  http://localhost:3000/api/v1/conformity-score`}
        ...
        <article className="rounded-xl border border-amber-200 dark:border-amber-900/50 bg-amber-50/60 dark:bg-amber-900/15 p-4">
          ...
          <ul className="text-xs text-amber-800/80 dark:text-amber-200/80 mt-2 space-y-1 list-disc pl-5 leading-relaxed">
            <li>Les clés sont hashées en SHA-256 — la valeur en clair n'est affichée qu'une seule fois à la création.</li>
...
function Endpoint({ method, path, desc }: { method: string; path: string; desc: string }) {
  return (
    <div className="flex items-start gap-2 bg-gray-50/60 dark:bg-slate-800/40 rounded-lg p-2">
      <span className="text-xs font-bold bg-emerald-600 text-white px-2 py-0.5 rounded shrink-0">{method}</span>
      <span className="text-primary-500 dark:text-accent-300 font-medium">{path}</span>
      <span className="text-gray-600 dark:text-gray-400 font-sans flex-1">— {desc}</span>
    </div>
  );
}
