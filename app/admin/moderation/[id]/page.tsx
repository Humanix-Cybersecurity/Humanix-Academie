      ...
      <div className="space-y-4 min-w-0">
          ...
          <div className="flex items-start gap-4">
            ...
            <div className="flex-1">
              ...
              <p className="text-xs text-gray-500 mt-3">
                ...
                <br />
                Soumis le {m.submittedAt?.toLocaleString("fr-FR")}
                <br />
                Slug : <code>{m.slug}</code>
                <br />
                SHA-256 :{" "}
                <code className="font-mono">{shortHash(m.contentHash)}</code>
              </p>
            </div>
          </div>
        </div>
        ...
        {payload.episodes.map((ep, i) => (
          <div key={i} className="card mb-3">
            ...
            <p className="text-xs uppercase tracking-wide text-gray-500 font-bold mt-2">
              Scénario
            </p>
            <div className="bg-gray-50 rounded-xl p-3 my-2 whitespace-pre-line text-sm">
              {ep.scenario}
            </div>

            <p className="text-xs uppercase tracking-wide text-gray-500 font-bold mt-3">
              Choix
            </p>
              ...
              {ep.choices.map((c) => (
                <li key={c.id} className="bg-white border rounded-xl p-2">
                  <p className="font-medium">
                    ...
                    >
                      {c.outcome.toUpperCase()} · {c.points >= 0 ? "+" : ""}
                      {c.points}
                    </span>
                    {c.label}
                  </p>
                  ...
                  <ul className="text-xs space-y-0.5 mt-1">
                    {q.choices.map((c) => (
                      <li
                        key={c.id}
                        className={
                          c.correct ? "text-success font-bold" : "text-gray-600"
                        }
                      >
                        {c.correct ? "✓ " : "○ "}
                        {c.label}
