...
export default async function ModerationPage() {
  ...
  const recent = await db.marketplaceModule.findMany({
    ...
    take: 10,
    include: { author: { select: { name: true, email: true } }, reviewedBy: { select: { name: true } } },
  });
    ...
    <div className="max-w-6xl mx-auto px-4 py-10">
      ...
      <div className="card mb-6 bg-primary-50 border-primary-500/30">
        ...
        <ul className="text-sm text-gray-700 space-y-1 list-disc pl-5">
          ...
          <li>Pédagogie : scénario crédible, choix clairs, débrief apportant de la valeur</li>
          <li>Pas de marketing produit déguisé / pas de promotion de marque</li>
          ...
          {queue.map((m) => (
            <div key={m.id} className="card">
              <div className="flex items-start gap-4">
                ...
                <div className="flex-1 min-w-0">
                  <div className="flex items-center gap-2 mb-1 flex-wrap">
                    ...
                    <span className="text-xs bg-gray-100 px-2 py-0.5 rounded-full">{m.category}</span>
                    <span className="text-xs bg-gray-100 px-2 py-0.5 rounded-full">{m.difficulty}</span>
                  </div>
                  ...
                  <p className="text-xs text-gray-500">
                    ...
                    <br />
                    Soumis {m.submittedAt?.toLocaleDateString("fr-FR")} · {m.submittedAt?.toLocaleTimeString("fr-FR")}
                    <br />
              ...
              <div className="mt-3 pt-3 border-t border-gray-100 flex flex-wrap gap-2 items-center">
                <Link href={`/admin/moderation/${m.id}`} className="btn-secondary text-sm py-2 px-4">
      ...
      <h3 className="text-xl font-bold text-primary-500 mb-3">🗄️ Historique récent</h3>
          ...
          <tbody>
            {recent.map((m) => (
              <tr key={m.id} className="border-b last:border-0">
                <td className="py-3 font-medium">{m.emoji} {m.title}</td>
                <td className="py-3 text-gray-600 text-xs">{m.author.name ?? m.author.email}</td>
                <td className="py-3">
                  {m.status === "APPROVED" ? (
                    <span className="text-xs bg-green-100 text-green-700 px-2 py-0.5 rounded-full font-bold">APPROUVÉ</span>
                  ) : (
                    <span className="text-xs bg-red-100 text-red-700 px-2 py-0.5 rounded-full font-bold">REFUSÉ</span>
                  )}
                </td>
                <td className="py-3 text-gray-500 text-xs">{m.reviewedBy?.name ?? "—"}</td>
                <td className="py-3 text-gray-400 text-xs">{m.reviewedAt?.toLocaleDateString("fr-FR")}</td>
              </tr>
            ))}
          </tbody>
        </table>
