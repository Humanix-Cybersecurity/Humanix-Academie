...
export default async function AdminPage() {
  ...
  const [users, saisons, allProgress] = await Promise.all([
    ...
    db.progress.findMany({
      ...
      include: {
        user: { select: { id: true, name: true, email: true, service: true, isActive: true } },
        episode: true,
        saison: true,
      },
    }),
  ...
  const averageXpPerEpisode = allProgress.length === 0 ? 0 : Math.round(
    allProgress.reduce((s, p) => s + (p.score || 0), 0) / allProgress.length,
  );
  ...
  const masteryAverage = activeUsers.length === 0
    ? 0
    : Math.round(
        activeUsers.reduce((s, u) => s + (u.riskScore ?? 50), 0) / activeUsers.length,
      );
  ...
  const teamProgress = activeUsers.map((u) => {
    ...
    return {
      ...
      lastActivity: last?.completedAt ? formatRelativeDate(last.completedAt) : null,
    };
  });
  ...
  const weeklyActivity = days.map((d) => {
    ...
    const count = allProgress.filter((p) => p.completedAt && p.completedAt >= d && p.completedAt < next).length;
    return { day: d.toLocaleDateString("fr-FR", { weekday: "short" }), completions: count };
  });
