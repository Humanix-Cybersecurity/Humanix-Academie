...
import LegalLayout, { LegalSection, LegalSubsection, LegalTable, LegalHighlight } from "@/components/legal/LegalLayout";

export const metadata = {
  title: "Politique de confidentialité — Humanix-Cybersecurity",
  description: "Comment nous traitons vos données personnelles, en application du RGPD.",
};
...
export default function ConfidentialitePage() {
  return (
    ...
    >
      ...
      <LegalSection id="collecte" num="2" title="Quand collectons-nous vos données ?">
        <p>Nous collectons des données dans 4 cas :</p>
        <ol>
          <li><strong>Visite du site</strong> humanix-cybersecurity.fr (cookies techniques uniquement)</li>
          <li><strong>Demande de contact</strong> via formulaire ou email</li>
          <li><strong>Utilisation de la plateforme Humanix Académie</strong> en tant qu'apprenant ou administrateur d'une organisation cliente</li>
          <li><strong>Prestation de service</strong> : formation, audit, accompagnement RSSI</li>
        </ol>
      </LegalSection>

      <LegalSection id="donnees" num="3" title="Quelles données et pour quelle finalité ?">
        ...
        <LegalSubsection num="3.2" title="Plateforme Humanix Académie">
          <LegalHighlight variant="info">
            <strong>En tant qu'éditeur, nous agissons comme sous-traitant</strong> au sens de l'article 28 du RGPD pour le compte de votre employeur (ou de l'organisation qui vous a inscrit). Le responsable du traitement de vos données d'apprentissage est votre employeur ; nous traitons les données <em>sur ses instructions documentées</em> via un contrat de sous-traitance (DPA).
          </LegalHighlight>
          <LegalTable
        ...
        <LegalSubsection num="3.4" title="Prestations (formation, audit, conseil)">
          <LegalTable
            headers={["Donnée", "Finalité", "Durée"]}
            rows={[
          ...
          <LegalHighlight variant="warning">
            <strong>Données techniques sensibles produites lors d'un audit / pentest</strong> (vulnérabilités, identifiants découverts, configurations) : transmises au Client exclusivement, par canal chiffré (TLS 1.3), jamais diffusées à des tiers. Au-delà de 3 ans, elles sont supprimées de manière irréversible ou restituées au Client sur demande, sauf obligation légale (litige, requête judiciaire).
          </LegalHighlight>
          <p className="text-sm">
            <strong>Tests de phishing simulé</strong> sur les collaborateurs du Client : ces tests ne sont menés qu'avec autorisation écrite explicite du responsable RH ou de la sécurité du Client, et ne contiennent aucune manipulation dolosive personnelle. Les résultats individuels ne sont communiqués qu'à des fins pédagogiques agrégées ou ciblées de formation, jamais à des fins de sanction.
          </p>
        </LegalSubsection>
      </LegalSection>
      ...
      <LegalSection id="acces" num="4" title="Qui peut accéder à vos données ?">
        <ul>
          <li><strong>En interne</strong> : le Président de Humanix-Cybersecurity uniquement (entreprise solo)</li>
          <li><strong>Sous-traitants techniques</strong> :
            <ul>
              <li><strong>Scaleway SAS</strong> (France 🇫🇷) — hébergement, sauvegardes, infrastructure</li>
              <li><strong>Olinda SAS</strong> exploitant le service <strong>Qonto</strong> (France 🇫🇷) — compte de paiement professionnel</li>
              <li><strong>Dougs SAS</strong> (France 🇫🇷) — expertise comptable, facturation</li>
              <li><strong>Resend</strong> (UE, sous Clauses Contractuelles Types CE 2021/914) — emails transactionnels (lien magique de connexion, alertes)</li>
            </ul>
          </li>
          <li><strong>Tiers institutionnels</strong> : administration fiscale, URSSAF, en cas d'obligation légale</li>
        </ul>
        <LegalHighlight variant="success">
          <strong>Aucun transfert hors UE</strong> n'est effectué pour les données traitées dans le cadre de la plateforme Humanix Académie. Si un transfert devait avoir lieu, il serait encadré par les clauses contractuelles types de la Commission européenne.
        </LegalHighlight>
      </LegalSection>

      <LegalSection id="connecteurs" num="4 bis" title="Connecteurs sortants activés par le client">
        <p>
          La plateforme propose des connecteurs et des formats d'export permettant à votre employeur (responsable du traitement) d'envoyer vos données de progression vers ses propres outils internes : outil GRC <strong>CISO Assistant</strong>, SIEM <strong>Microsoft Sentinel</strong> ou <strong>Splunk</strong>, format <strong>OSCAL</strong> (NIST), endpoint <strong>SCIM v2</strong> pour synchronisation d'annuaire, webhooks signés HMAC-SHA256.
        </p>
        <ul>
          <li>Ces connecteurs ne sont <strong>jamais activés sans action explicite</strong> de votre employeur (génération d'une clé API ou configuration d'un webhook depuis la console admin).</li>
          <li>Aucune <strong>donnée personnelle brute</strong> (PII) n'est transmise par ces flux : seuls des liens d'accès aux artefacts (qui restent sous notre contrôle d'accès) et des métriques agrégées par tenant.</li>
          <li>Une fois les données arrivées dans les outils du responsable de traitement, elles relèvent de <strong>sa propre responsabilité</strong> et de sa propre politique de confidentialité.</li>
          <li>Toutes les exportations sont journalisées (event <code>evidence.exported</code>) et accessibles dans l'audit trail tenant.</li>
        </ul>
      </LegalSection>
      ...
      <LegalSection id="droits" num="6" title="Vos droits">
        ...
        <ul>
          ...
          <li><strong>Effacement</strong> (droit à l'oubli) : supprimer vos données dans les limites prévues par la loi</li>
          <li><strong>Limitation du traitement</strong> : geler le traitement</li>
          <li><strong>Portabilité</strong> : récupérer vos données dans un format structuré</li>
          <li><strong>Opposition</strong> : refuser le traitement (notamment prospection)</li>
          <li><strong>Retrait du consentement</strong> à tout moment, sans effet rétroactif</li>
          <li><strong>Définir des directives post-mortem</strong> sur le sort de vos données</li>
        </ul>
        <p>
          Pour exercer vos droits, écrivez à <a href="mailto:rgpd@humanix-cybersecurity.fr">rgpd@humanix-cybersecurity.fr</a> ou par courrier au siège social. Nous répondons dans un délai maximal de 1 mois.
        </p>
        <LegalHighlight variant="info">
      ...
      <LegalSection id="cnil" num="7" title="Réclamation auprès de la CNIL">
        <p>
          Si vous estimez que vos droits ne sont pas respectés, vous pouvez introduire une réclamation auprès de la Commission nationale de l'informatique et des libertés (CNIL), 3 Place de Fontenoy — TSA 80715 — 75334 PARIS CEDEX 07. <a href="https://www.cnil.fr" rel="noopener noreferrer" target="_blank">cnil.fr</a>
        </p>
      </LegalSection>

      <LegalSection id="securite" num="8" title="Sécurité">
        <p>
          Humanix-Cybersecurity met en œuvre des mesures techniques et organisationnelles adaptées pour protéger vos données : chiffrement en transit (TLS), chiffrement au repos (Scaleway), authentification forte des accès administrateur, principe du moindre privilège, journalisation des accès, sauvegardes chiffrées. Le détail est disponible sur notre page <Link href="/securite">Sécurité &amp; Conformité</Link>.
        </p>
      </LegalSection>
