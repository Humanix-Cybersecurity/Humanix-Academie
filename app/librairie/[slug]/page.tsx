...
export default async function ArticleReadPage({ params }: { params: Promise<{ slug: string }> }) {
  ...
  db.libraryArticle.update({ where: { id: article.id }, data: { viewCount: { increment: 1 } } }).catch(() => {});
  ...
  return (
    <div className="max-w-3xl mx-auto px-4 py-10 animate-fadeIn">
      <Link href="/librairie" className="text-sm text-gray-500 hover:text-primary-500 mb-4 inline-block">
      ...
      <article className="card">
        <div className="flex items-start gap-4 mb-6 pb-6 border-b border-gray-100">
          ...
          <div className="flex-1">
            <h1 className="text-2xl sm:text-3xl font-extrabold text-primary-500">{article.title}</h1>
      ...
      {related.length > 0 && (
        <section className="mt-10">
          <h2 className="text-xl font-bold text-primary-500 mb-4">📚 À lire ensuite</h2>
          <div className="grid sm:grid-cols-3 gap-4">
            {related.map((r) => (
              <Link key={r.id} href={`/librairie/${r.slug}`} className="card hover:scale-[1.02] hover:shadow-md transition">
                <p className="text-3xl mb-2">{r.emoji}</p>
                <h3 className="font-bold text-primary-500 text-sm mb-1">{r.title}</h3>
                <p className="text-xs text-gray-500">⏱ {r.readTimeMinutes} min</p>
              </Link>
            ))}
          </div>
        </section>
